...
async function main() {
  ...
  function erDot(mmd) {
    ...
    const relations = [...mmd.matchAll(/^\s+(\w+) (\|\||\|o)\.\.o\{ (\w+) : "([^"]+)"/gm)];
    ...
    for (const [, parent, optional, child, field] of relations) dot += `${parent} -> ${child} [label=${quote(field)}, taillabel=${quote(optional === '||' ? '1' : '0..1')}, headlabel="0..N", labeldistance=2, dir=none];\n`;
    return dot + '}';
  }
  ...
  function compile(title, subtitle, files, filename, introductory) {
    ...
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)}</title><style>${stylesheet}</style></head><body><main><header class="cover"><p class="eyebrow">PowerPOS Pioneers</p><h1>${escape(title)}</h1><p class="subtitle">${escape(subtitle)}</p><p class="meta">Versión documental 1.1 · 16 de septiembre de 2026</p><p>${escape(introductory)}</p></header><nav aria-label="Contenido"><h2>Contenido</h2>${toc}</nav>${chapters.map((c) => c.html).join('')}</main><button class="print" onclick="window.print()">Imprimir o guardar como PDF</button></body></html>`;
  ...
  compile('Documentación académica y técnica', 'Requerimientos, diseño, operación y validación', [
    ...
    '06-diccionario-de-datos.md','07-api.md','08-instalacion-y-operacion.md','09-manual-de-usuario.md','10-pruebas-y-trazabilidad.md','11-brechas-y-evolucion.md','referencias/catalogo-api.md',
